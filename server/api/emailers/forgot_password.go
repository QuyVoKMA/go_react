...
// Tạo một struct rổng với tên sendMail
type sendMail struct {}

// tạo một interface chứa đối tượng SendResetPassword với 5 string vào, trả về một struct EmailRespone
...
func (s *sendMail) SendResetPassword(ToUser string, FromAdmin string, Token string, Sendgridkey string, AppEnv string) (*EmailResponse, error) {
	h := hermes.Hermes{
		Product: hermes.Product{
			Name: "QuyVo",
			Link: "https://quyvo.com",
		},
	}
	var forgotUrl string
	if os.Getenv("APP_ENV") == "production" {
		forgotUrl = "https://quyvo.com/resetpassword/" + Token //this is the url of the frontend app
	} else {
	...
	email := hermes.Email{
		Body: hermes.Body{
			Name: ToUser,
			Intros: []string{
				"Welcome to QuyVo! Good to have you here.",
			},
	...
	from := mail.NewEmail("QuyVo", FromAdmin)
